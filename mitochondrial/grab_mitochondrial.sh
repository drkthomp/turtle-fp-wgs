...
for SAMPLE in "$@";
do
BAM=${DATADIR}/${SAMPLE}/alignments/bwa_${SAMPLE}_CheMyd_sorted.bam

# NC_000886.1 is the name of the mitochondrial contig
samtools view --write-index --reference ${REF} -o bwa_${SAMPLE}_CheMyd_sorted_MITO.bam ${BAM} NC_000886.1
date
done

...
cat(paste("\n", sample, "\n"))
readcounts <- getReadCountsFromBAM(BAMFiles = sample,
                                   sampleNames = paste(sample),
                                   WL = windowsize,
                                   refSeqName = reference.scaffolds)
cat("\n Formatting read count outputs... \n")
counts.df <- data.frame(seqnames=seqnames(readcounts),
...
write.table(counts.mt, file = paste0(output.path, "readcounts_", windowsize, "BP_", sample, ".txt"), 

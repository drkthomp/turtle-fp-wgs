...
nice.seg.plot <-
  function(x.segmented,
           ...
           colorChrom = FALSE,
           save = TRUE) { # color chrom not recommending for display
    ratio <-
...
cnvAnalysis <- function(x) {
  ...
  if (!is.null(gr)) { # ensure that we can proceed
    ...
    segplot(segmented, lwd = 1, ylim = c(-5, 5), plot.type = "w", pt.cols = c("#999999", "#909090"), pt.cex = 1)
    dev.off()
    ...
    title <- paste( # turtle tissue FP - segmented CNVs
      str_replace(filter(sample_data, sample_name == tum_name)$turtle, "_kidney", ""),
    ...
    file_title <- paste0( # tum_normal FP - segmented CNVs(.pdf)
      tum,
      "_",
      filter(sample_data, sample_name == tum_name)$normal,
      " FP - segmented CNVs"
    )
    ...
    if ((!exists("doNiceSegPlot")) | doNiceSegPlot) { # nice seg plot takes a while, sometimes want to not rerun
      ...
      nice.seg.plot(
        ...
        indicateChrom = TRUE, # don't indicate the chromosomes (busy)
        colorChrom = FALSE, # def don't color the chromosomes (busy, no legend)
        annotate = FALSE, # do add small annotations
        title = title,

---
title: Sea Turtle FP Tumour CNV Analysis         
date: 26.01.2020                                       
organization: TCG Cambridge, Department of Veterinary Medicine 
email: mrs72@cam.ac.uk                                  
---
...
txdb = makeTxDbFromGFF("GCF_015237465.1_rCheMyd1.pri_genomic.gtf")
...
## Read in paired tumor and normal read count files
```{r Read In}
read_in <- function(x) {
  tumor_file <-
...
nice.seg.plot <-
  function(x.segmented,
    ...
    if(adjustForChrom){
      ...
      ranges(x.normalised.counts) <- IRanges(names=to_adjust$seqnames,
    ...
    if(colorChrom){
      ...
      levels(colors) <- c("#696969","#8b4513","#006400","#808000","#483d8b","#008b8b",
...
cnvAnalysis <- function(x) {
  ...
  if (!is.null(gr)) { # ensure that we can proceed
    ...
    segplot(segmented, lwd = 1, ylim = c(-5, 5))
    dev.off()
    ...
    cat("## Annotating")
    cnv <- cnvr(segmented)
    annotated <- annotateCnvs(cnv, txdb)
    write(annotated$gene_id %>% unlist(), paste0("data/", tum, "_cnvs.txt"))
    write.table(annotated, file=paste0("data/", tum, "_cnvs.tsv"), quote=FALSE, sep='\t', row.names = FALSE)
